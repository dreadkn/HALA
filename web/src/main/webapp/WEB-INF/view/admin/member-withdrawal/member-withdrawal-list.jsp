                                ...
                                <table id="dataTable" class="table table-sm table-bordered table-striped m-0" >
                                    <thead>
                                        <tr class="text-center text-nowrap">
                                            ...
                                            <th class="align-middle">탈퇴 회원명</th>
                                            <th class="align-middle">콘텐츠 등록</th>
                                            <th class="align-middle">광고 등록</th>
                                            <th class="px-1">가입일</th>
                                            <th class="px-1">탈퇴일</th>
                                            <th class="align-middle"></th>
                                        </tr>
                                    </thead>
                                </table>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods: {
            write: function () {
                location.href = contextPath + "/admin/member-withdrawal/form";
            },
                ...
                deleteConfirm(function(result)
                {
                    if (result)
                    {
                        ...
                        self.selected.forEach(function(id){
                            axios.delete("/admin/member-withdrawal/" + id, {data: {}}).then(function (res) {
                                if (++chkCnt >= chkLength)
                                {
                                    alertDeleted();
                                    setTimeout(function () {
...
<script>
    $(function () {
        dataTable = $('#dataTable').DataTable($.extend(true, dataTableOption, {
            ...
            ajax: {url: '/admin/member-withdrawal/list'},
            order: [[1, "desc"]],
            columns: [
                {
                    data: 'id',
                ...
                {data: 'withdrawalDate', className: 'text-center', render(data){
                        return dateFormat(data, 'YYYY.MM.DD');
                    }},
                {data: null, className: 'text-center', orderable: false, render: function (data, type, row) {
                    var url = contextPath + "/admin/member-withdrawal/" + row.id;
        ...
        $('form').submit(function (event) {
            ...
            dataTable.ajax.url(contextPath + '/admin/member-withdrawal/list' + getSearch()).load();
        });
