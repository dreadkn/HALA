                ...
                <div class="col-12">
                    ...
                    <div class="row mt-5 mb-3">
                        ...
                        <div class="col-auto">
                            ...
                            <button type="button" class="btn btn-primary " @click="write">구독 정책 등록</button>
                        </div>
                    </div>
                                ...
                                <table id="dataTable" class="table table-sm table-bordered table-striped m-0" >
                                    <thead>
                                        <tr class="text-center text-nowrap">
                                            ...
                                            <th class="align-middle">적용 기간</th>
                                            <th class="align-middle">콘텐츠 등록</th>
                                            <th class="align-middle">광고 1개월 등록</th>
                                            <th class="align-middle">구독기간</th>
                                            <th class="align-middle">요금</th>
                                            <th class="align-middle"></th>
                                        </tr>
                                    </thead>
                                </table>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods: {
            write: function () {
                location.href = contextPath + "/admin/subscribe-policy/form";
            },
                ...
                deleteConfirm(function(result)
                {
                    if (result)
                    {
                        ...
                        self.selected.forEach(function(id){
                            axios.delete("/admin/subscribe-policy/" + id, {data: {}}).then(function (res) {
                                if (++chkCnt >= chkLength)
                                {
                                    alertDeleted();
                                    setTimeout(function () {
...
<script>
    $(function () {
        dataTable = $('#dataTable').DataTable($.extend(true, dataTableOption, {
            ...
            ajax: {url: '/admin/subscribe-policy/list'},
            order: [[1, "desc"]],
            columns: [
                {
                    data: 'id',
                ...
                {data: 'startDate', className: 'text-center', render(data, type, row){
                    return dateFormat(data,'YYYY.MM.DD') + ' ~ ' + dateFormat(row.endDate,'YYYY.MM.DD');
                    }},
                {data: 'contentCount', className: 'text-center', render(data){
                    return data + '건';
                    }},
                {data: 'adCount', className: 'text-center', render(data){
                        return data + '건';
                    }},
                {data: 'periodType', className: 'text-center', render(data){
                    return data.value;
                    }},
                {data: 'price', className: 'text-center'},
                {data: null, className: 'text-center', orderable: false, render: function (data, type, row) {
                    var url = contextPath + "/admin/subscribe-policy/" + row.id;
        ...
        $('form').submit(function (event) {
            ...
            dataTable.ajax.url(contextPath + '/admin/subscribe-policy/list' + getSearch()).load();
        });
