...
<div id="vue-app" class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4 class="m-0 text-dark">이메일 발송 목록</h4>
                </div>
                <div class="d-none col-sm-6 d-md-block">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item active">이메일 발송 관리</li>
                        <li class="breadcrumb-item active">이메일 발송 관리</li>
                        <li class="breadcrumb-item active">이메일 발송 목록</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    ...
                    <div class="row mt-5 mb-3">
                        ...
                        <div class="col-auto">
                            ...
                            <button type="button" class="btn btn-primary " @click="write">Email 발송</button>
                        </div>
                    </div>
                                ...
                                <table id="dataTable" class="table table-sm table-bordered table-striped m-0" >
                                    <thead>
                                        <tr class="text-center text-nowrap">
                                            ...
                                            <th class="align-middle">알림 구분</th>
                                            <th class="align-middle">알림명</th>
                                            <th class="align-middle">발송자</th>
                                            <th class="align-middle">발송일</th>
                                            <th class="align-middle">대상자</th>
                                            <th class="align-middle"></th>
                                        </tr>
                                    </thead>
                                </table>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods: {
            write: function () {
                location.href = contextPath + "/admin/email/form";
            },
                ...
                deleteConfirm(function(result)
                {
                    if (result)
                    {
                        ...
                        self.selected.forEach(function(id){
                            axios.delete("/admin/email/" + id, {data: {}}).then(function (res) {
                                if (++chkCnt >= chkLength)
                                {
                                    alertDeleted();
                                    setTimeout(function () {
    ...
    $(function () {
        dataTable = $('#dataTable').DataTable($.extend(true, dataTableOption, {
            ...
            columns: [
                ...
                {data: 'alarmTarget', className: 'text-center', render(data){
                    return data.value;
                    }},
                {data: 'title', className: 'text-center'},
                {data: 'sender', className: 'text-center'},
                {data: 'sendDate', className: 'text-center', render(data){
                    return dateFormat(data);
                }},
                {data: 'receiversCount', className: 'text-center'},
                {data: null, className: 'text-center', orderable: false, render: function (data, type, row) {
                    var url = contextPath + "/admin/email/" + row.id;
        ...
        $('form').submit(function (event) {
            ...
            dataTable.ajax.url(contextPath + '/admin/email/list' + getSearch()).load();
        });
