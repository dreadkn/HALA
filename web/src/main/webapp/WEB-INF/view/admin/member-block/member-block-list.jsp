                                ...
                                <table id="dataTable" class="table table-sm table-bordered table-striped m-0" >
                                    <thead>
                                        <tr class="text-center text-nowrap">
                                            ...
                                            <th class="align-middle">차단 회원명</th>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods: {
            write: function () {
                location.href = contextPath + "/admin/member-block/form";
            },
                ...
                deleteConfirm(function(result)
                {
                    if (result)
                    {
                        ...
                        self.selected.forEach(function(id){
                            axios.delete("/admin/member-block/" + id, {data: {}}).then(function (res) {
                                if (++chkCnt >= chkLength)
                                {
                                    alertDeleted();
                                    setTimeout(function () {
...
<script>
    $(function () {
        dataTable = $('#dataTable').DataTable($.extend(true, dataTableOption, {
            ...
            ajax: {url: '/admin/member-block/list'},
            order: [[1, "desc"]],
            columns: [
                {
                    data: 'id',
                ...
                {data: null, className: 'text-center', orderable: false, render: function (data, type, row) {
                    var url = contextPath + "/admin/member-block/" + row.id;
        ...
        $('form').submit(function (event) {
            ...
            dataTable.ajax.url(contextPath + '/admin/member-block/list' + getSearch()).load();
        });
