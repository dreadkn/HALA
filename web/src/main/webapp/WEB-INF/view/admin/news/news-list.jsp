                ...
                <div class="col-12">
                    ...
                    <div class="row mt-5 mb-3">
                        ...
                        <div class="col-auto">
                            ...
                            <button type="button" class="btn btn-primary " @click="write">뉴스 등록</button>
                        </div>
                    </div>
...
<script>
    ...
    var vm = new Vue({
        ...
        methods: {
            write: function () {
                location.href = contextPath + "/admin/news/form";
            },
            addCheck: function (id) {
                ...
                alertConfirm(function(result){
                    if (result)
                    {
                        axios.get(contextPath + "/admin/news/check/" + id, {data: {}}).then(function (data) {
                ...
                deleteConfirm(function(result)
                {
                    if (result)
                    {
                        ...
                        self.selected.forEach(function(id){
                            axios.delete("/admin/news/" + id, {data: {}}).then(function (res) {
                                if (++chkCnt >= chkLength)
                                {
                                    alertDeleted();
                                    setTimeout(function () {
...
<script>
    $(function () {
        dataTable = $('#dataTable').DataTable($.extend(true, dataTableOption, {
            ...
            columns: [
                ...
                {data: null, className: 'text-center', orderable: false, render: function (data, type, row) {
                    var url = contextPath + "/admin/news/" + row.id;

                    return "<button class='btn btn-sm btn-outline-primary row-detail mr-1 text-nowrap' onclick='location.href=\""+url+"\"'>상세</button>"
                        + "<button class='mr-1 btn btn-sm btn-outline-primary row-detail-window text-nowrap' onclick='window.open(\""+url+"\")'>새창</button>"
        ...
        $('form').submit(function (event) {
            ...
            dataTable.ajax.url(contextPath + '/admin/news/list' + getSearch()).load();
        });
