        	...
        	if (typeof data[i].divider !== 'undefined') {
				...
				if (typeof data[i].subMenu !== 'undefined') {
					...
					if (typeof data[i].img !== 'undefined')
						element += '<img src="' + data[i].img + '" class="mr-3"/> ';
				...
				if (typeof data[i].action !== 'undefined') {
					...
					$sub
						...
						.on('click', createCallback($action, data[i].type, data[i].item));
				}
				$menu.append($sub);
				if (typeof data[i].subMenu != 'undefined') {
...
var createCallback = function(func, type, item) {
    return function(event) { func(event, currentContextSelector, type, item) };
}
