package com.inpply.web.project.admin.member.block.repository;
...
@Repository
public class BlockMemberQueryRepository extends Querydsl4RepositorySupport {

    public BlockMemberQueryRepository() {
        super(Member.class);
    }
    ...
    public Page<MemberDto.ListItem> findAll(Pageable pageable, Search search) {
        ...
        JPAQuery<MemberDto.ListItem> query =
                select(Projections.fields(MemberDto.ListItem.class,
                        ...
                        .where(member.enabled.eq(false).and(member.isWithdrawal.eq(false)));
